...
<p>Confidence intervals for random effects parameters are not being correctly reported by tidy.glmmTMB() function (https://github.com/bbolker/broom.mixed/issues/108) in negative binomial regressions (correctly reported for regression with zero-inflated component). Thus they were computed using glmmTMB::confint() function using Wald method as visible below.</p>
...
<p>Confidence intervals for random effects parameters are not being correctly reported by tidy.glmmTMB() function (https://github.com/bbolker/broom.mixed/issues/108) in negative binomial regressions (correctly reported for regression with zero-inflated component). The CI for Random Effects (Intercept) in the previous tables should be ignored. CI for this variable was computed using glmmTMB::confint() function using Wald method as visible below.</p>
